    ...
    for i in range(0, stage_length):
        for j in range(0, stage_nums[i]):
            ...
            tmp_index = TrainedNN(inputs, labels,
                                  threshold[i], use_threshold[i], core_nums[i], train_step_nums[i], batch_size_nums[i],
...
def optimize_storage(do_compare, do_record, threshold, use_threshold, data_part_distance, learning_percent,
                     distribution):
    ...
    for i in range(STORE_NUMBER):
        # test_set_x.append(tmp_data.iloc[i, 0])
        # test_set_y.append(tmp_data.iloc[i, 1])
        train_set_x.append(tmp_data.iloc[i, 0])
        train_set_y.append(tmp_data.iloc[i, 1])
    ...
    if do_compare == 1 or do_compare == 2:
        ...
        if do_record:
            with open('optimization_result.csv', 'w') as csvFile:
        ...
        for i in range(to_store_data.shape[0]):
            pre_data = to_store_data.iloc[i, 0]
        ...
        with open("store_performance/" + pathString[distribution] + "/optimization/" + str(
            data_part_distance) + "_" + str(learning_percent) + ".json", "w") as jsonFile:
            json.dump(result, jsonFile)

        if do_record:
            with open('insert_result.csv', 'w') as csvFile:
                csv_writer = csv.writer(csvFile)
                for i in store_data:
                    csv_writer.writerow([i])
    ...
    if do_compare == 0 or do_compare == 2:
        ...
        with open("store_performance/" + pathString[distribution] + "/no_optimization/"
                  + str(learning_percent) + ".json", "w") as jsonFile:
            json.dump(result, jsonFile)
...
def show_help_message(msg):
    help_message = {
        'command': 'python learned_index.py -d <Distribution> [-p] [Percent] '
