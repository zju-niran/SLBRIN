...
from src.spatial_index.common_utils import get_min_max, Region, binary_search_less_max
from src.spatial_index.geohash_utils import Geohash


class SBRIN:
    def __init__(self, version=None, size=None, threshold_number=None, threshold_length=None, difflen=None,
                 geohash=None, regular_pages=None):
        # meta page
        ...
        # regular page
        # blkgh: 改动：只存单个值，不再是range，对应values
        # blkghlen: 新增，blk geohash的实际length
        # blkreg: BRIN-Spatial有，blk的region
        # blknum: BRIN有，blk的数据量
        # blkindex: BRIN有，对应itemoffsets
    ...
    def build_by_quadtree(self, quad_tree, geohash_length):
        ...
        self.regular_pages = [RegularPage(blkgh=self.geohash.geohash_to_int(item["geohash"]),
                                          blkghlen=len(item["geohash"]),
                                          blkreg=item["region"],
    ...
    def build(self, data_list, geohash_length, region, threshold_number, data_precision):
        """
        构建SBRIN
        1. 初始化第一个block
        ...
        while len(init_block_list):
            ...
            if cur_block[3] >= self.threshold_number and cur_block[1] < self.threshold_length:
                ...
                for i in range(4):
                    ...
                    tmp_right_index = binary_search_less_max(data_list, 2, min_geohash_value, tmp_left_index,
                                                             right_index)
        ...
        self.regular_pages = [RegularPage(blkgh=blk[0] << max_geohash_length - blk[1],
                                          blkghlen=blk[1],
    ...
    def point_query(self, point):
        ...
        """
        return self.regular_pages[self.binary_search_less_max(point >> self.difflen, 0, self.size)]
    ...
    def range_query_old(self, point1, point2, window):
        ...
        """
        i = self.binary_search_less_max(point1 >> self.difflen, 0, self.size)
        j = self.binary_search_less_max(point2 >> self.difflen, i, self.size)
        if i == j:
    ...
    def range_query(self, point1, point2):
        ...
        # i, j = 0, 0
        i, j = 1, self.binary_search_less_max(origin_geohash_list[0][0], 0, self.size)
        origin_geohash_list[0][0] = j
        while i < size1:
            if self.regular_pages[j].blkgh > origin_geohash_list[i][0]:
    ...
    def binary_search_less_max(self, x, left, right):
        ...
        while left <= right:
            mid = (left + right) // 2
            if self.regular_pages[mid].blkgh == x:
                return mid
            elif self.regular_pages[mid].blkgh < x:
                left = mid + 1
            else:
                right = mid - 1
        return right

# TODO: blknum是pages的offset，含义要纠正
class RegularPage:
    def __init__(self, blkgh, blkghlen, blkreg, blknum, blkindex, blknn=None):
        self.blkgh = blkgh
    ...
    @staticmethod
    def init_by_dict(d: dict):
        return RegularPage(blkgh=d['blkgh'],
                           blkghlen=d['blkghlen'],
                           blkreg=d['blkreg'],
                           blknum=d['blknum'],
                           blkindex=d['blkindex'],
